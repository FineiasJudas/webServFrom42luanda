...
/*
    Add variaveis especificas para cada interpertador
*/
    ...
    CgiType cgi)
{
    ...
    if (cgi == CGI_PHP){addPhpVars(env);}
    //else if (cgi == CGI_PYTHON){addPythonVars(env);} 
    return env;
}
                              ...
                              const ServerConfig& config)
{
    ...
    */
    ...
    /*
     obter a extencao do arquivo a executar
    */

    std::string extension = Utils::getExtension(script_path);

    CgiType cgi;
    if (extension == ".php")      cgi = CGI_PHP;
    else if (extension == ".py")  cgi = CGI_PYTHON;
    else if (extension == ".lua") cgi = CGI_LUA;
    else if (extension == ".rb")  cgi = CGI_RUBY;
    else  cgi = CGI_UNKNOWN;
    ...
    */
    ...
    // ------------------ BUILD ENV -----------------------
    std::vector<std::string> env_strings = buildEnv(req, script_path, config, cgi);
