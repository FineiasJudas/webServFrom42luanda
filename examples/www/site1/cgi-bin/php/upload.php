...
if (!isset($_FILES['file'])){respond(400, 'Nenhum arquivo enviado');}
if ($_FILES['file']['error'] !== UPLOAD_ERR_OK){respond(400, 'Erro no upload: ' . $_FILES['file']['error']);}

if ($_FILES['file']['size'] > $MAX_SIZE){respond(413, 'Arquivo excede o limite de 10MB');}
...
if (!is_dir($uploadDir)) {mkdir($uploadDir, 0755, false);}
...
if (!empty($pathInfo['extension'])) {$dest .= "." . $pathInfo['extension'];}

// - Mover arquivo -
if (!move_uploaded_file($_FILES['file']['tmp_name'], $dest)){respond(500, 'Falha ao salvar o arquivo');}
