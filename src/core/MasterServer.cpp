...
MasterServer::~MasterServer()
{
    ...
    for (std::map<int, ServerConfig *>::iterator it = listenFdToServers.begin();
         it != listenFdToServers.end(); ++it){close(it->first);}
}
...
int MasterServer::parsePortFromListenString(const std::string &s) const
{
    ...
    if (colon == std::string::npos){portstr = s;}
    else{portstr = s.substr(colon + 1);}
    if (portstr.empty() || !Utils::isNumber(portstr)){return (-1);}
    ...
    for (size_t i = 0; i < servers.size(); ++i){
        ...
        if (sc.listen.size() > 0){
            ...
            if (std::find(ports.begin(), ports.end(), port) == ports.end()){
                ...
                else {Logger::log(Logger::ERROR, "Falha ao criar um listen para a porta " + Utils::toString(port));}
            }
            else{
...
bool MasterServer::isListenFd(int fd) const {return (listenFdToServers.count(fd) > 0);}
...
ServerConfig *MasterServer::selectServerForRequest(const Request &req, int listenFd)
{
    ...
    if (it == listenFdToServers.end()){return (NULL);}
...
void MasterServer::handleRead(int clientFd)
{
    ...
    while (HttpParser::hasCompleteRequest(conn->getInputBuffer())){
        Request req;
        if (!HttpParser::parseRequest(conn->getInputBuffer(), req, max_body)){break;}
        ...
        conn->setServer(sc);
        std::string connHdr;
        if (req.headers.count("Connection")){connHdr = req.headers.at("Connection");}

        if (connHdr == "close"){conn->setCloseAfterSend(true);}
        else if (req.version == "HTTP/1.0" && connHdr != "keep-alive"){conn->setCloseAfterSend(true);}
        else{conn->setCloseAfterSend(false);}
    ...
    if (processed > 0){poller.modifyFd(clientFd, EPOLLOUT);}
}
...
void MasterServer::handleWrite(int clientFd)
{
    ...
    if (conn->shouldCloseAfterSend() && out.empty()){return closeConnection(clientFd);}
